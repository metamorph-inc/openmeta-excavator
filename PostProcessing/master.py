...
def main():
    ...
    #--------Length-----------------------------------------------
    bucketCyl_uri = 'Excavator_.bucketCyl_Length'
    filter.append(bucketCyl_uri)
    armCyl_uri = 'Excavator_.armCyl_Length'
    filter.append(armCyl_uri)
    boomCyl_uri = 'Excavator_.boomCylL_Length'
    ...
    #--------Load------------------
    Load_uri = 'WorldForce.yForce'
    filter.append(Load_uri)
    #----Torque at the undercarriage-----------
    ...
    if nextState != -1:
        #print 'next Time : {0}'.format(nextState)
    ...
    #----------------------------------------------------------------------
    numStates = pp.global_max(State_uri)    
    enterState_time = 0
    maxBoomOvershoot = 0;
    maxArmOvershoot = 0;
    ...
    for i in xrange(1, int(numStates)+1):
        ...
        print 'desiredArm:{0}'.format(desiredArm)
        ...
        if (armOvershoot>maxArmOvershoot):
            #print 'armOvershoot:{0}'.format(armOvershoot)
        ...
        #---- Settling Time -------------        
        changeState = pp.get_time(State_uri,2)
        print 'changeState is:{0}'.format(changeState)
        ...
        else: maxBoomSettleTime = find_first_max_violation_reverse(pp, boomCylLength_uri, desiredBoom[0], tol, enter_index, exit_index)
        ...
        else: maxArmSettleTime = find_first_max_violation_reverse(pp, armCylLength_uri, desiredArm[0], tol, enter_index, exit_index)
    ...
    if any(v>0 for v in a.itervalues()):
        ...
        minVal = -1
        culprit = 'none'
    maxLoad = pp.get_data_by_index(Load_uri, minVal)
    ...
    metrics.update({'boomCylLength_unit': {'value': pp.get_data_by_time(boomCylLength_uri, pp.global_max_time(boomCylLPressureA_uri))[0], 'unit': 'm'},
                    ...
                    'boomCylLPressureA': {'value': pp.global_abs_max(boomCylLPressureA_uri)*0.00001, 'unit': 'bar'},                    
                    'armCylPressureA': {'value': pp.get_local_max(armCylPressureA_uri,initTime,-1)*0.00001, 'unit': 'bar'},	
                    'bucketCylPressureA': {'value': pp.global_abs_max(bucketCylPressureA_uri)*0.00001, 'unit': 'bar'},
                    'boomCylLPressureB': {'value': pp.global_abs_max(boomCylLPressureB_uri)*0.00001, 'unit': 'bar'},                    
                    'armCylPressureB': {'value': pp.get_local_max(armCylPressureB_uri,initTime,-1)*0.00001, 'unit': 'bar'},	
                    'bucketCylPressureB': {'value': pp.global_abs_max(bucketCylPressureB_uri)*0.00001, 'unit': 'bar'},                   				
                    ...
                    'maxBoomOvershoot':{'value': maxBoomOvershoot, 'unit':'m'},
                    'maxArmOvershoot':{'value': maxArmOvershoot, 'unit':'m'},
                    'maxBucketOvershoot':{'value': maxBucketOvershoot, 'unit':'m'},
                    ...
                    'cylinder_exceed_Relief' : {'value': culprit, 'unit': 'null'},
                    'tipOver' : {'value': pp.global_abs_max(tipOver_uri),'unit':'N-m'},
                    'maxLoad' : {'value': -1*maxLoad,'unit':'kg'},
                    })
